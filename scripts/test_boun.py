...
if __name__ == '__main__':
    ...
    device = 'cpu' # CPU mode for testing without GPU
    ...
    for model_path_CDDPM in model_path_CDDPMs:
        ...
        model_number = model_filename.replace('model', '').replace('.pt', '')  # e.g., '1000000'
        if len(model_number) >= 3 and int(model_number[:3]) % 1000 == 100:  # Check first 3 digits
