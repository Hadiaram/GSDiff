...
if __name__ == '__main__':
    ...
    dataloader_train = DataLoader(dataset_train, batch_size=batch_size, shuffle=True, num_workers=8,
                            drop_last=True, pin_memory=True, persistent_workers=True, prefetch_factor=2)  # Optimized for Linux/GPU
    ...
    dataloader_val = DataLoader(dataset_val, batch_size=batch_size_val, shuffle=False, num_workers=8,
                            drop_last=False, pin_memory=True, persistent_workers=True, prefetch_factor=2)  # Optimized for Linux/GPU
    ...
    '''TEMPORARILY DISABLED - Validation rendering hangs with fully connected edges from converted data'''
    '''TODO: Re-enable after implementing proper edge generation based on spatial proximity'''
    ...
    if resume_step > 0:
        ...
        if os.path.exists(checkpoint_path) and os.path.exists(optim_path):
            print(f"Resuming from step {resume_step}...")
            model.load_state_dict(torch.load(checkpoint_path, map_location=device))
            optimizer.load_state_dict(torch.load(optim_path, map_location=device))
            print(f"Successfully loaded checkpoint from step {resume_step}")
        else:
            print(f"Warning: Checkpoint files not found at step {resume_step}. Starting from scratch.")
            resume_step = 0
    ...
    while step < total_steps:
        ...
        '''loss backward with mixed precision'''
        ...
        if step % interval == 0:  # Re-enabled with corrected path to stage-2 model
            ...
            '''calculate FID, KID. '''
            # TEMPORARILY DISABLED: FID/KID calculation requires ground truth validation images
            ...
            current_Fid = 0.0  # Placeholder
            current_Kid = 0.0  # Placeholder
            print('step: ', step, 'FID: DISABLED', 'KID: DISABLED')
