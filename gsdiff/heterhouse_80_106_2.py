...
class TransformerLayer(nn.Module):
    def __init__(self, d_model):
        ...
        self.corner_norm = nn.InstanceNorm1d(d_model)
        self.corner_global_attn = MultiHeadAttention(4, d_model)
        self.corner_cross_attn = MultiHeadCrossAttention(4, d_model, d_embed=256)
        self.corner_feedforward = nn.Sequential(
            nn.Linear(d_model, d_model * 4),
            nn.ReLU(),
            nn.Linear(d_model * 4, d_model)
...
class TopoHeterHouseModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.d_model = 512
    ...
    def forward(self, corners_withsemantics_t, global_attn_matrix, t, bb_semantics_embedding, bb_semantics_padding_mask):
        ...
        '''cross attn mask'''
        cross_attn_mask = torch.bmm(global_attn_matrix[:, :, 0:1].float(), bb_semantics_padding_mask.transpose(1, 2).float())
