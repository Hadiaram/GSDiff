...
def create_featuremaps_for_directory(test_data_dir, output_dir):
    """
...
def create_withboundary_files(test_data_dir, output_dir):
    """
        ...
        try:
            ...
            graph_withboundary = {
                'corner_list_np_normalized_padding_withsemantics': graph.get(
                    ...
                    np.random.randn(53, 16).astype(np.float32)
                ),
                'padding_mask': graph.get(
                    'padding_mask',
                    np.ones((53, 1), dtype=np.uint8)
                )
            }
...
Examples:
    ...
    # Create feature maps
    create_featuremaps_for_directory(args.test_dir, args.feature_dir)

    # Create withboundary files if requested
    if args.create_withboundary:
        create_withboundary_files(args.test_dir, args.withboundary_dir)
